...
C
      IP = I+1
      IM = I-1
      IF(S(I) .ceq. S(IP)) IP = I+2
      IF(S(I) .ceq. S(IM)) IM = I-2
...
C
      IF((EI11.ceq.0.0) .OR. (EI22.ceq.0.0)) THEN
...
C------ allow for doubled points
        IF((DX1.ceq.0.0) .AND. (DY1.ceq.0.0)) THEN
         DX1 = X(I) - X(I-2)
         DY1 = Y(I) - Y(I-2)
        ENDIF
        IF((DX2.ceq.0.0) .AND. (DY2.ceq.0.0)) THEN
         DX2 = X(I) - X(I+2)
         DY2 = Y(I) - Y(I+2)
        ENDIF
C
     ...
     &         / SQRT((DX1**2 + DY1**2) * (DX2**2 + DY2**2))
        ...
        IF(IPRINT.ceq.2) WRITE(*,1100) I, X(I), Y(I), ANGL
...
C
        ...
        IF((CRSP.ceq.0.0) .AND. (DOTP.ceq.0.0)) THEN
         ASEG = 0.0
        ELSE
