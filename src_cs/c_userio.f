...
C
 10   WRITE(*,1000) PROMPT(1:NP)
      READ (*,1010) CHAR
      IF(CHAR.ceq.'y') CHAR = 'Y'
      IF(CHAR.ceq.'n') CHAR = 'N'
      IF((CHAR.cne.'Y') .AND. (CHAR.cne.'N')) GO TO 10
C
      LINPUT = CHAR .ceq. 'Y'
      RETURN
C
...
C
        IF(K.ceq.KSPACE) THEN
C------- just skip this space
         K = K+1
         GO TO 9
        ENDIF
C
        IF(K.ceq.KCOMMA) THEN
